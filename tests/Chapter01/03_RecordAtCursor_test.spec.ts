...
test("Search Playwright Courses", async ({  }) => {
    ...
    await test.step("Verify content on screen" , async () => {
        await expect(page.getByRole('link',{ name : "#1 Playwright Tutorial Full Course 2026 | Playwright Testing Tutorial"})).toBeVisible();
    });
    await context.close();
});